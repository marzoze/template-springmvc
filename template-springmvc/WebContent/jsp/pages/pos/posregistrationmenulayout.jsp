		...
		<ul class="dropdown-menu" style="display: block; position: static; width: 196px">
			<c:choose>
				<c:when test="${not empty posregistrationid}">
					<li <c:if test="${sideMenuActive eq 'policyinfo'}">class="active"</c:if>><a href="<c:url value="/"/>registration/policyinfo?regid=${posregistrationid}"><strong><fmt:message key="lbl.Registration.header.policyinfo" bundle="${com}" /></strong></a></li>
				</c:when>
				<c:when test="${not empty param.regid}">
					<li <c:if test="${sideMenuActive eq 'policyinfo'}">class="active"</c:if>><a href="<c:url value="/"/>registration/policyinfo?regid=${posregistrationid}"><strong><fmt:message key="lbl.Registration.header.policyinfo" bundle="${com}" /></strong></a></li>
				</c:when>				
				<c:otherwise>
					<li <c:if test="${sideMenuActive eq 'policyinfo'}">class="active"</c:if>><a href="<c:url value="/"/>registration/policyinfo"><strong><fmt:message key="lbl.Registration.header.policyinfo" bundle="${com}" /></strong></a></li>
				</c:otherwise>
			</c:choose>				
			<li class="divider"></li>
			<c:choose>
				<c:when test="${not empty posregistrationid}">
					<li <c:if test="${sideMenuActive eq 'postransaction'}">class="active"</c:if>><a href="<c:url value="/"/>registration/postransaction?regid=${posregistrationid}"><strong><fmt:message key="lbl.Registration.header.postransaction" bundle="${com}" /></strong></a></li>
				</c:when>
				<c:when test="${not empty param.regid}">
					<li <c:if test="${sideMenuActive eq 'postransaction'}">class="active"</c:if>><a href="<c:url value="/"/>registration/postransaction?regid=${posregistrationid}"><strong><fmt:message key="lbl.Registration.header.postransaction" bundle="${com}" /></strong></a></li>
				</c:when>				
			...
			<c:choose>
				<c:when test="${not empty postransactionid}">
					<li <c:if test="${sideMenuActive eq 'posdocument'}">class="active"</c:if>><a href="<c:url value="/"/>registration/posdocument?regid=${posregistrationid}"><strong><fmt:message key="lbl.Registration.header.posdocument" bundle="${com}" /></strong></a></li>
				</c:when>				
				<c:otherwise>
					<li <c:if test="${sideMenuActive eq 'posdocument'}">class="active"</c:if>><a href="javascript: alert('Please Complete Previous Step ')"><strong><fmt:message key="lbl.Registration.header.posdocument" bundle="${com}" /></strong></a></li>
				</c:otherwise>
			</c:choose>			
		</ul>
	</div>
...
$('document').ready(function(){	
	...
	$.ajax({
		type:'POST',
		url:postrackingcontextpath+'/registration/getRegistrationStatus?postransactionid=' + postransactionid,
	...
	$.ajax({
		type:'POST',
		url:postrackingcontextpath+'/registration/getTransactionStatus?postransactionid=' + postransactionid,
		success: function(data) {
