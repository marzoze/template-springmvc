...
<body>
	...
	<c:url value="/registration/transaction" var="transaction"/>
			...
			<div class="widget-header">
	  		...
	  		<div class="widget-content">
	  			...
	  			<div class="form-actions">  					
		    		...
		    		<button type="button" class="btn btn-warning" id="btnSaveContinuePosTransaction"><fmt:message key="btn.saveContinue" bundle="${com}" /></button>		    		
	...
	<script type="text/javascript">		
		...
		postracking.form.init({
		    ...
		    fn:function(){
		    	...
		    	postracking.autocomplete("posTransactionTypeCode",postrackingcontextpath + "/registration/getTranCodeAutoComplete");

		    	// Autocomplete tran desc
		    	postracking.autocomplete("posTransactionTypeDesc",postrackingcontextpath + "/registration/getTranDescAutoComplete");
		    	...
		    	$("#dialogDeleteMessage").dialog({  	    		
					...
					buttons: {
						...
						'Yes':function(){					
							...
							$.ajax({
								type:'POST',
								url:postrackingcontextpath+'/registration/deleteAttachment?attachId=' + selectedRows,	
								data:params,							
		    	...
		    	$("#gridPosTransactionType").jqGrid({
					datatype: "json",
					url: postrackingcontextpath+"/registration/getPosTransactionType?posNumber=" + $('#posNumber').val(),
					...
					loadComplete : function(){
						...
						if (transactionId != '') {
							$.ajax({
								type:'POST',
								url:postrackingcontextpath+'/registration/getServiceSelected?transactionId=' + transactionId,
								success: function(data){																							
		    	...
		    	$("#gridPosTranTypeSelected").jqGrid({
					datatype: "json",
					url: postrackingcontextpath+"/registration/proccesTransactionType?serviceId=" + idsOfSelectedRows_ +
						...
						var ids = jQuery("#gridPosTranTypeSelected").jqGrid('getDataIDs');						  							  			
			  			for ( var i = 0; i < ids.length; i++ ) {
							...
							$('#' + idDeleteTranType).live('click',function(event) {	
								...
								$.ajax({
									type:'POST',
									url:postrackingcontextpath + '/registration/deleteTransactionType?transactionTypeNo=' + tranTypeNo,																							
									success:function(data){
										if(data.result == '200'){
											...
											*/
								    		...
								    		$("#gridPosTranTypeSelected").setGridParam({
									    		url 			: postrackingcontextpath+"/registration/proccesTransactionType?serviceId=" + idsOfSelectedRows +
																		"&posNumber=" + $('#posNumber').val() +
							...
							$('#' + idTextarea).live('change',function(event){
								...
								$.ajax({
									type:'POST',
									url:postrackingcontextpath+'/registration/onChangeRemark?tranTypeNo=' + tranTypeNo 
							...
							$('#' + idReceivedDate).live('change',function(event){								
								...
								$.ajax({
									type:'POST',
									url:postrackingcontextpath+'/registration/onChangeReceivedDate?tranTypeNo=' + tranTypeNo 
							...
							$.ajax({
								type:'POST',
								url:postrackingcontextpath+'/registration/checkUploadedDocument?tranTypeNo=' + tranTypeNo,
								success: function(data){																							
					...
					subGridRowExpanded: function(subgrid_id, row_id) {
						...
						$("#"+subgrid_table_id).jqGrid({
							datatype: "json",
							url		: postrackingcontextpath+"/registration/findDocumentTransactionSubGrid?serviceId=" + row_id,
							...
							gridComplete : function(){	
								multiColor(subgrid_table_id);
					  			...
					  			for ( var i = 0; i < ids.length; i++ ) {
					  				...
					  				var be1 = '<form id=\"'+formId+'\" method="post" enctype="multipart/form-data" action="/postracking/registration/documentUpload">' +					  						  
									...
									$.ajax({
										type:'POST',
										url:postrackingcontextpath+'/registration/checkUploadedDocumentSubGrid?tranTypeNo=' + mainGrid.transactionNumber 
													+ '&index=' + ids[i],
										success: function(data){
											if(data.result == '200'){	
										        ...
										        var be3x = '<a href="' + postrackingcontextpath + '/registration/downloadAttachment?'+ paramDownloadDoc +'\" title="Download" id="linkDownload"><img src="${pageContext.request.contextPath}/images/download.png" /></a>' +
												...
												$('#' + xButtonRemove).live('click',function(event){
													...
													$.ajax({
														type:'POST',
														url:postrackingcontextpath + '/registration/deleteDocument?fileName=' + data.fileName 
														...
														success:function(data){
															if(data.result == '200'){										
																$('#'+subgrid_table_id).jqGrid('setRowData', ids[splitId[1].substring(0,1) - 1], {																
																		inputFile 	: '<form id=\"formUploadDoc_'+btnId+'_'+row_id+'\" method="post" enctype="multipart/form-data" action="/postracking/registration/documentUpload">' +					  						  
									...
									$('#' + buttonId).live('click',function(event){														
										...
										$.ajax({
												type:'POST',
												url:postrackingcontextpath + '/registration/documentUpload?posNumber=' + posNumber 
														...
														$('#' + buttonRemove).live('click',function(event){
															$.ajax({
																...
																success:function(data){
																	if(data.result == '200'){										
																		$('#'+subgrid_table_id).jqGrid('setRowData', ids[splitId[1] - 1], {																
																			inputFile 	: '<form id=\"formUploadDoc_'+btnId+'\" method="post" enctype="multipart/form-data" action="/postracking/registration/documentUpload">' +					  						  
		    	...
		    	$("#gridAttachment").jqGrid({
					datatype: "json",
					url: postrackingcontextpath+"/registration/getAttachment?attachId=" + $('#attachId').val(),
					...
					gridComplete : function(){
						...
						for ( var i = 0; i < ids.length; i++ ) {
							...
							url = '<a href="registration/downloadAttachment?'+param+'\" class="btn btn-warning" style="color:white;">Download</a>'														
							var be = url;			  							  							  				
		    	...
		    	$("#btnSearchPosTransactionType").click(function(e) {					    		    			    
		    		$("#gridPosTransactionType").setGridParam({
			    		url 			: postrackingcontextpath+"/registration/getPosTransactionType?tranDesc=" + $('#posTransactionTypeDesc').val() +
			    								"&posNumber=" + $('#posNumber').val() +
			    								"&policyNumber=" +  $('#policyNumber').val()
		    		});
		    		$("#gridPosTransactionType").trigger("reloadGrid");
		    	...
		    	$("#btnSaveTransactionType").click(function(e){
			    	...
			    	$.ajax({
						type:'POST',
						url:postrackingcontextpath + '/registration/saveTransactionType',	
						data: {data:data},							
						success:function(data){
		    	...
		    	$("#deliveryToPos").loadOptions(postrackingcontextpath+"/registration/getEmployeePosDept",{
		    	...
		    	$("#btnSubmitPosRegistration").click(function() {
					$("#transactionForm").attr("action",postrackingcontextpath+"/registration/submit?page=2");
					$("#transactionForm").submit();	
				});

		    	$("#btnSaveContinuePosTransaction").click(function() {
					$("#transactionForm").attr("action",postrackingcontextpath+"/registration/saveContinueTransaction");
					$("#transactionForm").submit();	
				});

				$("#btnBacktoBranch").click(function() {
					$("#transactionForm").attr("action",postrackingcontextpath+"/registration/backtobranch?page=2");
					$("#transactionForm").submit();	
				});				
