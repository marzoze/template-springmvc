...
<body>
	...
	<c:url value="/registrationbrc" var="registration"/>
	  		...
	  		<div class="widget-content">
				...
				<c:import url="/jsp/pages/pos/posprogresstrackerbranch.jsp" />
	  			...
	  			<div class="form-actions">  					
		    		...
		    		<button type="button" class="btn btn-warning" id="btnSaveContinuePosPosDocument"><fmt:message key="btn.saveContinue" bundle="${com}" /></button>		    		
	...
	<script type="text/javascript">
		...
		postracking.form.init({
		    ...
		    fn:function(){
		    	...
		    	$("#grid").jqGrid({
					datatype: "json",
					url: postrackingcontextpath+"/registrationbrc/getPolicyInquiry",
					...
					gridComplete : function(){						
			  			...
			  			for ( var i = 0; i < ids.length; i++) {	
							...
							$('#'+id).live('click',function(event){
								...
								showLoading();														
								$.getJSON(postrackingcontextpath+"/registrationbrc/selectPolicy?", { 'policyId':btnId }, function(data){
								...
								$("#gridBenefitInfo").setGridParam({
						    		url 			: postrackingcontextpath+"/registrationbrc/getBenefitInfo?pId="+btnId,
		    	...
		    	$("#btnSearchPolicy").click(function(e) {			    	
		    		$("#grid").setGridParam({
			    		url 			: postrackingcontextpath+"/registrationbrc/getPolicyInquiry?search="+$('#search').val(),
			    		loadComplete	:function(){
			    			var ids = jQuery("#grid").jqGrid('getDataIDs');
							if(ids.length == ''){
								// do nothing
		    	...
		    	$("#gridBenefitInfo").jqGrid({
					datatype: "json",
					url: postrackingcontextpath+"/registrationbrc/getBenefitInfo?policyCode=" + $('#policyNumber').val(),
		    	...
		    	$("#btnSaveContinuePosPosDocument").click(function() {
					$("#registrationForm").attr("action",postrackingcontextpath+"/registrationbrc/saveContinuePolicyInfo");
					$("#registrationForm").submit();	
				});

		    	$("#btnSubmitPosRegistration").click(function() {
					$("#registrationForm").attr("action",postrackingcontextpath+"/registrationbrc/submit?page=1");
					$("#registrationForm").submit();	
				});  	
